...
`GET /api/v1/limit-events` accepts one optional `reason_keyword`, trimmed and matched as a literal substring of final `reason_text` with case-insensitive UTF-8 search. The parameter is bound in `query.go` and combined with date, stock, event and theme conditions before ordering and pagination. Empty keywords preserve existing behavior; other review endpoints reject nonempty reason keywords. No token splitting, wildcard expressions, semantic expansion, new table or dependency is introduced.
