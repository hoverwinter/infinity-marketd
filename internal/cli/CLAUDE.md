...
Subcommands: `bootstrap` (create schema), `status` (watermarks/health), `import-tdx-day` / `import-tdx-1m` / `import-tdx-5m` (with `--root`+`--code`, `--file`, and `--dry-run`), `import-tdx-vipdoc-zip`, `import-tdx-hq-day` (explicit online HQ daily import), `quote`, `quote-probe`, `quote-sweep`, `exquote-markets`, `exquote-count`, `exquote-instruments`, `exquote`, `exquote-bars`, `exquote-minute`, `exquote-history-minute`, `exquote-transactions`, `exquote-history-transactions`, and `exquote-history-bars`.
